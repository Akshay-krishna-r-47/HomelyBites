...
$pending_sql = "SELECT sa.*, u.email, u.name as user_real_name FROM seller_applications sa JOIN users u ON sa.user_id = u.user_id WHERE sa.status = 'Pending'";
                ...
                <table>
                    ...
                    <tbody>
                        <?php if ($pending_count > 0): ?>
                            ...
                            <tr>
                                <td>
                                    <strong><?php echo htmlspecialchars($row['business_name']); ?></strong><br>
                                    <span style="font-size: 0.8rem; color: #888;"><?php echo htmlspecialchars($row['user_real_name']); ?></span><br>
                                    <span style="font-size: 0.75rem; color: #aaa;">Type: <?php echo htmlspecialchars($row['kitchen_type']); ?></span>
                                </td>
                                <td>
                                    <?php echo htmlspecialchars($row['email']); ?><br>
                                    <span style="font-size: 0.8rem;"><?php echo htmlspecialchars($row['phone_number']); ?></span>
                                </td>
                                <td><span class="status-badge">Pending</span></td>
                                <td>
                                    <form action="admin_handle_seller.php" method="POST" style="display:inline;">
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="4" style="text-align:center; color: #999; padding: 40px;">No pending requests found.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
